import random
def deal_cards():
    cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10,10]
    card = random.choice(cards)
    return card

user_card = []
computer_card = []
computer_score = -1
user_score = -1
is_game_over = False

def calculate_points(cards):
    if sum(cards) == 21 and len(cards) == 2:
        return 0
    if 11 in cards and sum(cards) > 21:
        cards.remove (11)
        cards.append(1)
    return sum(cards)


for _ in range(2):
    user_card.append(deal_cards())
    computer_card.append(deal_cards())

while not is_game_over == True:

    user_score = calculate_points(user_card)
    computer_score = calculate_points(computer_card)
    print(f"Your cards: {user_card}, current score: {user_score}")
    print(f"Computer's first card: {computer_card[0]}")


    if user_score == 0 or computer_score == 0 or user_score > 21:
        is_game_over = True
    else:
        user_should_deal = input("Do you want to draw another card? (y/n): ")
        if user_should_deal == "y":
            user_card.append(deal_cards())
        else:
            is_game_over = True

while computer_score != 0 and computer_score < 17:
    computer_card.append(deal_cards())
    computer_score = calculate_points(computer_card)

def compare(u_score, c_score):
    if u_score == c_score:
        return "It's a draw."
    elif c_score == 0:
        return "Lose, opponent has a BlackJack"
    elif u_score == 0:
        return "Win, you have a BlackJack!"
    elif u_score > 21:
        return "You lost, went over the limit"
    elif c_score > 21:
        return "Computer lost, opponent went over the limit"
    elif u_score > c_score:
        return "You win!"
    else:
        return "Computer wins!"
    
print(compare(user_score, computer_score))